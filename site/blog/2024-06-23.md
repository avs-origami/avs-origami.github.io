...
honestly overkill. Instead, we use good ol' Unix Sockets along with +[https://crates.io/crates/passfd]a nifty
crate called passfd+[endurl] to transfer the file descriptors and set up the shared
memory pipeline.
...
[[bin]]
name = "server"
path = "src/server.rs"]```_[--------------------------------------------------------------------------------]_

Now that the setup is done, we can start doing some interesting things.
